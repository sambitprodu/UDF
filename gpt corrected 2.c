...
DEFINE_ADJUST(mass_transfer_rate, c_ms_P1, t_ms_P1, d_ms_P1, eqn)
{
    ...
    thread_loop_c(t_ms_P1, d_ms_P1)
    {
        ...
        if (FLUID_THREAD_P(t_P1) || FLUID_THREAD_P(t_P2))
        {
            begin_c_loop(c_ms_P1, t_ms_P1)
            {
                if (FLUID_THREAD_P(t_P1))
                {
                    ro_P1 = C_R(c_ms_P1, t_P1);
                    vof_P1 = C_VOF(c_ms_P1, t_P1);
                }

                if (FLUID_THREAD_P(t_P2))
                {
                ...
                {
                    source_mass_P1 = G * 3.14 * d * d * (1.0 - vof_P2);
                }
...
DEFINE_SOURCE(mass_P1, c_ms_P1, t_ms_P1, ds_ms_P1, eqn_ms_P1)
{
    ...
    thread_loop_c(t_ms_P1, d_ms_P1)
    {
        ...
        if (fabs(C_UDMI(c_ms_P1, t_ms_P1, 3)) > 0.0)
        {
            return 0.0;
        }
        ...
        {
            source_mass_P1 = -1.0 * C_UDMI(c_ms_P1, t_ms_P1, 3); /* If liquid phase is less than the minimum volume, then no phase transfer will be taking place */
        }
    }
...
DEFINE_SOURCE(mass_P2, c_ms_P2, t_ms_P2, ds_ms_P2, eqn_ms_P2)
{
    ...
    thread_loop_c(t_ms_P2, d_ms_P2)
    {
        ...
        if (fabs(C_UDMI(c_ms_P2, t_ms_P2, 4)) > 0.0)
        {
            return 0.0;
        }
        ...
        {
            source_mass_P2 = -1.0 * C_UDMI(c_ms_P2, t_ms_P2, 4); /* If liquid phase is less than the minimum volume, then no phase transfer will be taking place */
        }
    }
